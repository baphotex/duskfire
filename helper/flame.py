...
def lm(X,y):
    ...
    print(f"Fitting a Linear Regression on y vs X")
    X,y = np.array(X), np.array(y) 
    model.fit(X,y)

    score = model.score(X,y)
    print(f"R^2 of {np.round(score,4)*100}% on X and y.")
    w,b = model.coef_, model.intercept_
    
    return (model, score, w,b)


def lm_model_plot(X,y,byo=False,model=None,labels=None):
    ...
    model = model if model else lm(X,y)

    x = X.squeeze() 

    xlabel,ylabel,title=labels if labels else ("x-axis","y-axis","Linear Regression on y-vs-x")

    (m,score,w,b) = model
    w = w[0]
    
    fig, ax = plt.subplots()
    plt.scatter(x,y, s=12,color='darkgreen', label=r"data $\{(x_i,y_i)\}$")

    xplot = np.linspace(start=x.min(), stop=x.max()).reshape(-1,1)
    yplot = w * xplot + b
    plt.plot(xplot, yplot, label=r'linear fit $\hat{P}(y = 1)$', color='grey')
